...
1. **Auto-generated** (`src/tables/dictionary.json`) — ~19K entries extracted from the GeoNames/OSM registry by `scripts/generate-dictionary.js`. Only actual transliterations are included. Observation counts are category-weighted (airports/stations 3×, tourism 1.5×, shops 0.3×).
2. **Manual overrides** (`src/tables/dictionary-manual.json`) — Hand-curated translations (Bangkok, Thailand). NOT regenerated by calibrate. All manual entries get a hardcoded weight of 1.5 (`MANUAL_WEIGHT` in `dictionary.js`).

Manual entries always outrank auto-generated and algorithmic variants.

### Category Multipliers

Extraction scripts apply multipliers to observation counts based on the type of place:

**OSM categories** (`extract-osm.js`):
...
| `amenity` | 0.5× | Restaurants, banks |
| `shop` | 0.3× | Retail stores |

**GeoNames feature codes** (`extract-geonames.js`):

| Code | Multiplier | Examples |
|------|-----------|----------|
| `AIRP/AIRF/AIRB` | 3× | Airports, airfields |
| `RSTN` | 3× | Railroad stations |
| `PPLC` | 2× | Country capitals |
| `PPLA` | 1.5× | Province capitals |
| `MNMT/SHRN/TMPL` | 1.5× | Monuments, shrines, temples |
| Class `S` | 0.8× | Generic structures |
| Class `H/T` | 0.5× | Streams, terrain |

This ensures well-known place names (สุวรรณภูมิ → "suvarnabhumi") naturally outrank phonetic variants without manual overrides.
